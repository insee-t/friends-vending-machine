...
// Simple shared storage using a free service
const STORAGE_URL = 'https://api.jsonbin.io/v3/b'
const BIN_ID = '65f8a8e2dc74654018b0a123' // This will be created when you first run
const API_KEY = '$2a$10$your-api-key-here' // You'll need to get this from jsonbin.io

export default function MultiDevicePairingGame() {
  ...
  const loadUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (currentUserInList && currentUserInList.status === 'paired') {
          const pair = data.record?.currentPair
          if (pair) {
            setCurrentPair(pair)
            setGamePhase('paired')
            setTimeout(() => {
  ...
  const saveUsers = async (users: User[], pair?: Pair) => {
    try {
      const data = {
        users,
  ...
  useEffect(() => {
    ...
    if (gamePhase === 'waiting') {
      intervalRef.current = setInterval(async () => {
        ...
        // Check for pairing
        if (users.length >= 2) {
          const waitingUsers = users.filter((user: User) => user.status === 'waiting')
  ...
  const createPair = async (users: User[]) => {
    if (users.length >= 2) {
      ...
      const updatedUsers = allUsers.map((user: User) => {
        if (user.id === user1.id || user.id === user2.id) {
          return { ...user, status: 'paired' as const }
        }
        return user
      })
      ...
      setCurrentPair(pair)
      setGamePhase('paired')
      await saveUsers(updatedUsers, pair)
  ...
  const startNewGame = async () => {
    ...
    if (currentUser) {
      const updatedUsers = allUsers.filter((user: User) => user.id !== currentUser.id)
      setAllUsers(updatedUsers)
      await saveUsers(updatedUsers)
    }
