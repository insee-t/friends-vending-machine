...
export default function RealTabPairingGame() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
      channelRef.current = new BroadcastChannel('pairing-game')
        ...
        switch (type) {
          ...
          case 'USERS_PAIRED':
            ...
            setAllUsers(prev => prev.map(user => {
              if (user.id === data.user1.id || user.id === data.user2.id) {
  ...
  // Check for pairing when users change
  useEffect(() => {
    if (gamePhase === 'waiting' && allUsers.length >= 2) {
      const waitingUsers = allUsers.filter(user => user.status === 'waiting')
      if (waitingUsers.length >= 2) {
        // Small delay to ensure both users are registered
        setTimeout(() => {
          createPair()
        }, 1000)
      }
    ...
    <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-8 max-w-2xl mx-auto">
      <div className="text-center">
        ...
        <div className="bg-gray-400 bg-opacity-20 rounded-lg p-4 mb-6">
          ...
          <p className="text-gray-200 text-xs mt-1">
            <strong>BroadcastChannel:</strong> {typeof window !== 'undefined' && 'BroadcastChannel' in window ? 'Supported' : 'Not Supported'}
          </p>
        </div>
