...
interface User {
  ...
  nickname: string
  joinedAt: number
  status: 'waiting' | 'paired' | 'in-activity'
}
...
export default function RealTimePairingGame() {
  ...
  // Real-time sync with localStorage (simulates real-time updates)
  useEffect(() => {
      ...
      intervalRef.current = setInterval(() => {
        ...
        if (savedUsers) {
          try {
            const users = JSON.parse(savedUsers)
            const now = Date.now()
            // Filter out old users and update status
            const activeUsers = users.filter((user: User) => 
              now - user.joinedAt < 5 * 60 * 1000
            )
            
            // Always update to get latest state
            setAllUsers(activeUsers)
            ...
            if (currentUserInList && currentUserInList.status === 'paired') {
              // Find the pair for current user
              const pairData = localStorage.getItem('current-pair')
              if (pairData) {
              ...
              const waitingUsers = activeUsers.filter(user => user.status === 'waiting')
              if (waitingUsers.length >= 2) {
                // Check if current user is one of the waiting users
                const currentUserWaiting = waitingUsers.some(user => user.id === currentUser?.id)
                if (currentUserWaiting) {
                  pairUsers(waitingUsers)
                }
              }
            }
          } catch (error) {
  ...
  const pairUsers = (users: User[]) => {
    if (users.length >= 2) {
      ...
      // Update user statuses in localStorage first
