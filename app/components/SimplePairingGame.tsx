...
function ActivityScreen({ 
    ...
    if (userAnswer.trim() && !isSubmitted) {
      ...
      if (userFile) {
        try {
          ...
          formData.append('userId', pair.user1.id); // Simple version uses user1
      ...
      // For simple version, we'll just store locally
      localStorage.setItem(`question-answer-${pair.id}`, JSON.stringify({
  ...
  const handleActivitySubmit = async () => {
    if (userActivityAnswer.trim() && !isActivitySubmitted) {
      let fileUrl = null;
      ...
      if (userFile) {
        try {
          ...
          formData.append('file', userFile);
          formData.append('pairId', pair.id);
          formData.append('userId', pair.user1.id);
      ...
      // For simple version, we'll just store locally
      localStorage.setItem(`activity-answer-${pair.id}`, JSON.stringify({
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-lg border-2 border-white border-opacity-30 bg-white bg-opacity-10 text-white placeholder-white placeholder-opacity-60 focus:border-opacity-60 focus:outline-none text-lg resize-none"
            ...
            <div className="text-right mt-1">
              <span className="text-sm text-white opacity-60">{userAnswer.length}/200</span>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border-2 border-white border-opacity-30 bg-white bg-opacity-10 text-white focus:border-opacity-60 focus:outline-none"
              disabled={isSubmitted}
            />
            {userFileUrl && (
              <div className="mt-3">
                <p className="text-sm text-white opacity-80 mb-2">ไฟล์ที่เลือก:</p>
                {userFile?.type.startsWith('image/') ? (
                  <img src={userFileUrl} alt="Preview" className="max-w-xs rounded-lg" />
                ) : (
                  <p className="text-sm text-white opacity-80">{userFile?.name}</p>
                )}
              </div>
            )}
          </div>
            ...
            <button
              ...
              className={`px-6 py-3 rounded-lg font-semibold transition-all transform ${
                isSubmitted
                  ...
                  : userAnswer.trim()
                  ? 'bg-gradient-to-r from-purple-400 to-pink-400 text-white hover:from-purple-500 hover:to-pink-500 hover:scale-105'
      ...
      <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            />
            <div className="text-right mt-1">
              <span className="text-sm text-white opacity-60">{userActivityAnswer.length}/300</span>
            </div>
          </div>
          ...
          <div className="text-center">
            <button
              ...
              className={`px-6 py-3 rounded-lg font-semibold transition-all transform ${
                isActivitySubmitted
                  ? 'bg-green-500 text-white cursor-not-allowed'
                  : userActivityAnswer.trim()
                  ? 'bg-gradient-to-r from-green-400 to-emerald-400 text-white hover:from-green-500 hover:to-emerald-500 hover:scale-105'
                  : 'bg-gray-500 text-gray-300 cursor-not-allowed'
              }`}
              disabled={!userActivityAnswer.trim() || isActivitySubmitted}
            >
        ...
        <div className="text-center mt-6">
          <button
            ...
            className="bg-gradient-to-r from-green-400 to-emerald-400 text-white font-semibold py-3 px-6 rounded-lg hover:from-green-500 hover:to-emerald-500 transition-all transform hover:scale-105"
          >
            {showActivityAnswer ? 'ซ่อนคำตอบ' : 'ดูคำตอบ'} 👀
          </button>
        </div>
        ...
        {showActivityAnswer && (
          <div className="mt-6 bg-yellow-400 bg-opacity-20 rounded-lg p-4">
            <p className="text-yellow-200 text-center">
              🎯 ลองทำกิจกรรมนี้ด้วยกันและแบ่งปันประสบการณ์!
            </p>
          </div>
        )}
