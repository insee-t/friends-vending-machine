  ...
  socket.on('submit-answer', (data) => {
    ...
    if (pair) {
      console.log(`Pair details:`, {
        ...
        user1Nickname: pair.user1.nickname,
        user2Nickname: pair.user2.nickname
      });
      
      // Find the partner's socket ID - userId is the socket ID
      const partnerSocketId = pair.user1.id === userId ? pair.user2.id : pair.user1.id;
      ...
      // Send the answer to the partner using their socket ID
      socket.to(partnerSocketId).emit('receive-answer', {
        userId: userId,
        answer: answer,
        questionIndex: questionIndex
      });
...
// Helper function to get 6 random questions
function getRandomQuestions() {
  const shuffled = [...iceBreakingQuestions].sort(() => 0.5 - Math.random());
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    const { email, password, nickname, socialMediaHandle } = req.body;
    ...
    const user = await db.createUser({
      ...
      nickname: nickname.trim(),
      password,
      socialMediaHandle: socialMediaHandle?.trim() || null
    });
